...
% Solid concentration
s0.V0 = 3.8;
[csn0,csp0] = init_cs(p,s0.V0);
s0.c_n0 = csn0 * ones(1,p.Nr-1);
s0.c_p0 = csp0 * ones(1,p.Nr-1);

% Electrolyte concentration
ce0 = p.c_e;
s0.ce0 = p.c_e*ones(Nx,1);
data.c_e0 = s0.ce0;
...
p.D_s_n = p.D_s_n0 * exp(p.E.Dsn/p.R*(1/p.T_ref - 1/s0.T0));
p.D_s_p = p.D_s_n0 * exp(p.E.Dsp/p.R*(1/p.T_ref - 1/s0.T0));

% Kinetic reaction rate
p.k_n = p.k_n0 * exp(p.E.kn/p.R*(1/p.T_ref - 1/s0.T0));
p.k_p = p.k_p0 * exp(p.E.kp/p.R*(1/p.T_ref - 1/s0.T0));
...
[c_ss_n,t,c_nx] = lsim(sys_n,I,t,s0.c_n0);     % Anode Particle
[c_ss_p,t,c_px] = lsim(sys_p,I,t,s0.c_p0);     % Cathode Particle
...
for k = 1:NT
    ...
    dfca_n = electrolyteAct(cen_bar(k),s0.T0,p);
    dfca_s = electrolyteAct(ces_bar(k),s0.T0,p);
    dfca_p = electrolyteAct(cep_bar(k),s0.T0,p);
    ...
    s(k).SOC_p = 3/p.c_s_p_max * trapz(p.r_vec,p.r_vec.^2.*c_p(k,:)');
    ...
    s(k).c_e = c_e_nb(:,k);
    s(k).c_e_bcs = [ce0n(k); cens(k); cesp(k); ce0p(k)];
    s(k).c_nx = c_nx(k,:);
    s(k).c_px = c_px(k,:);
    s(k).c_ss_n = c_ss_n(k);
    s(k).c_ss_p = c_ss_p(k);
